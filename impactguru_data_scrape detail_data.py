...
__code_name__ = "impact_guru_fundraiser_detail_data_scraper"
__version__ = "1.0"
...
class Fundraiser:
    ...
    def __init__(self):
        self.output_dict = {'Section_Name':[],'Campaign_Title':[],'Campaign_By':[],'Beneficiary':[],'Total_Amount':[],
                            'Funds_Raised':[],'Campaign_URL':[],'Location':[],'Account_Name':[],'Account_Number':[],
                            'IFSC_code':[]}
        ...
        for sections in sections_type:
            ...
            try:
                ...
                for body in body_sections:
                    ...
                    res = requests.get(campaign_url).text
                    html_data = html.fromstring(res)
                    total_amount = html_data.xpath('//span[@class="box-stick__color-light"]/text()')
                    campaign_money_raised = html_data.xpath('//span[@class="custom-raisedAmount"]/text()')
                    beneficiary_name = html_data.xpath('//div[@class="description"]/div/text()')
                    location = beneficiary_name[3]
                    try:
                        bank_details = html_data.xpath('//span[@class="b-level"]/text()')
                        account_number = str(bank_details[0]).strip(' ')
                        account_name = str(bank_details[1]).strip(' ')
                        IFSC_code = str(bank_details[2]).strip(' ')
                    except:
                    ...
                    self.output_dict['Funds_Raised'].append(str(campaign_money_raised[0]).strip(' '))
                    self.output_dict['Beneficiary'].append(str(beneficiary_name[5]).strip(' '))
                    self.output_dict['Total_Amount'].append(str(total_amount[0]).strip('of '))
                    self.output_dict['Location'].append(str(location).strip(' '))
                    self.output_dict['Account_Number'].append(account_number)
                    self.output_dict['Account_Name'].append(account_name)
                    self.output_dict['IFSC_code'].append(IFSC_code)
    ...
    def write_output(self):
        ...
        writer = pd.ExcelWriter(dir_path + '\Output\Impact_Guru_detail_data' + str(now) + '.xlsx')
